...
class BookTrain extends Ticket{
...
class User{
    ...
    static void showSingleUser(User user){
        ...
        if(user.trainBooked == null && user.busBooked == null){
            System.out.printf("%n%30s%sYou have no Journeys planned...Returning to Dashboard..%s" , "" , ANSI_RED , ANSI_YELLOW);
        }
        if(user.trainBooked != null) user.trainBooked.showJourney();
        if(user.busBooked != null) user.busBooked.showJourney();
    }
    ...
    static User ticketBooking(User user , String type){
        ...
        // creating ticket 
        if(type.equals("train")) user.trainBooked = new BookTrain();
        else user.busBooked = new BookBus();
        ...
        switch(option){
            case 1:
                ...
                if(type.equals("train")){
                    user.trainBooked = generateTicket(user.trainBooked , BookingSys.VizToHydStations , BookingSys.VizToHydStationsDis , type);
                }else{
                    user.busBooked = generateTicket(user.busBooked , BookingSys.VizToHydStations , BookingSys.VizToHydStationsDis , type);
                }
                break;
            case 2:
                BookingSys.showSingleRoute(BookingSys.HowToPuriStations);
                if(type.equals("train")){
                    user.trainBooked = generateTicket(user.trainBooked , BookingSys.HowToPuriStations , BookingSys.HowToPuriStationsDis , type);
                }else{
                    user.busBooked = generateTicket(user.busBooked , BookingSys.HowToPuriStations , BookingSys.HowToPuriStationsDis , type);
                }
                break;
            case 3:
                BookingSys.showSingleRoute(BookingSys.ChennaiToMysoreStations);
                if(type.equals("train")){
                    user.trainBooked = generateTicket(user.trainBooked , BookingSys.ChennaiToMysoreStations , BookingSys.ChennaiToMysoreStationsDis , type);
                }else{
                    user.busBooked =  generateTicket(user.busBooked , BookingSys.ChennaiToMysoreStations , BookingSys.ChennaiToMysoreStationsDis , type);
                }
                break;
             default:
                System.out.printf("%30s" , "");
    ...
    static Ticket generateTicket(Ticket ticket , String [] route , int [] distances , String type){
        ...
        if(!(opt == 'y' || opt == 'Y')){
            ...
            System.out.printf("%n%30s%n%30s", "" , "");
            System.out.println(ANSI_GREEN + "Ticket Booked Succesfully " + ANSI_YELLOW);
        }
        return ticket;
...
class BookingSys{
    ...
    static String []  VizToHydStations = {"Visakhapatnam" , "Samlkota" , "Rajahmundry" , "Vijayawada" , "Khammam" , "Warangal" , "Secundrabad"} ;
